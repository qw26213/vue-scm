...
<script>
import { getAggregate, getReceiptTable, getReceiptQueryConfList, getDefaultReceiptQueryConf, saveReceiptQueryConf, delReceiptTemplate, exportReceiptBook, printReceiptBook } from '@/api/table'
...
export default {
  ...
  created() {
    ...
    getAggregate('receipt').then(res => {
      this.levellist = res.data
      getDefaultReceiptQueryConf().then(res => {
  ...
  methods: {
    ...
    delItem(id) {
      delReceiptTemplate(id).then(res => {
    ...
    saveTemplate(obj) {
      ...
      saveReceiptQueryConf(data).then(res => {
        if (res.data.errorCode === '0') {
          this.getTemplateList()
    ...
    getTemplateList() {
      getReceiptQueryConfList().then(res => {
        this.templatelist = res.data.data || []
      })
    },
    exportBook() {
      exportReceiptBook(this.listQuery)
    },
    printBook() {
      printReceiptBook(this.listQuery).then(res => {
    ...
    getList() {
      ...
      getReceiptTable(this.listQuery).then(res => {
        this.listLoading = false
