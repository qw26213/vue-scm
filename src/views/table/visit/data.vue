...
<script>
import { getAggregate, getVisitTable, getVisitQueryConfList, getDefaultVisitQueryConf, saveVisitQueryConf, delVisitTemplate, exportVisitBook, printVisitBook } from '@/api/table'
...
export default {
    ...
    created() {
      ...
      getAggregate('visit').then(res => {
        this.levellist = res.data
        getDefaultVisitQueryConf().then(res => {
    ...
    methods: {
        ...
        saveTemplate(obj) {
            ...
            saveVisitQueryConf(data).then(res => {
                if (res.data.errorCode == 0) {
                    this.getTemplateList()
        ...
        getTemplateList() {
            getVisitQueryConfList().then(res => {
                this.templatelist = res.data.data || []
            })
        },
        exportBook() {
            exportVisitBook(this.listQuery)
        },
        printBook() {
            printVisitBook(this.listQuery).then(res => {
        ...
        getList() {
            ...
            getVisitTable(this.listQuery).then(res => {
                this.listLoading = false
