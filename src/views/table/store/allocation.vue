<template>
  <div class="app-container">
    <div class="filterDiv">
      ...
      <warehouseList key-type="outWarehouseId" place-txt="源仓库" ctr-type="list" :select-id="listQuery.outWarehouseId" @selectChange="selectChange" />
      <label class="label ml10">目标仓库</label>
      <warehouseList key-type="inWarehouseId" place-txt="目标仓库" ctr-type="list" :select-id="listQuery.inWarehouseId" @selectChange="selectChange" />
...
<script>
import { getAggregate, getAllocationTable, getAllocationQueryConfList, getDefaultAllocationQueryConf, saveAllocationQueryConf, delAllocationTemplate, exportAllocationBook, printAllocationBook } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    delItem(id) {
      delAllocationTemplate(id).then(res => {
    ...
    saveTemplate(obj) {
      ...
      saveAllocationQueryConf(data).then(res => {
        if (res.data.errorCode === '0') {
          this.getTemplateList()
    ...
    getTemplateList() {
      getAllocationQueryConfList().then(res => {
        this.templatelist = res.data.data || []
      })
    },
    exportBook() {
      exportAllocationBook(this.listQuery)
    },
    printBook() {
      printAllocationBook(this.listQuery).then(res => {
    ...
    getList() {
      ...
      getAllocationTable(this.listQuery).then(res => {
        this.listLoading = false
