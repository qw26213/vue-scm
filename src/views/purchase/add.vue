    ...
    <div class="dataTable">
      <el-form :inline="true" label-position="right" label-width="72px" style="width: 100%; margin-top:0px;">
        ...
        <el-form-item label="付款到期日:" prop="paymentDueDate">
          <el-date-picker v-model="temp.paymentDueDate" :editable="false" type="date" placeholder="付款到期日" size="small" :clearable="false" value-format="yyyy-MM-dd" />
        </el-form-item>
        ...
        <el-form-item label="使用预付:" prop="advPayAmount">
          <el-input v-model="temp.advPayAmount" size="small" placeholder="使用预付" disabled />
        </el-form-item>
    ...
    <div class="contentDiv">
      <el-table :data="tableData" border fit highlight-current-row style="width: 100%;" size="small" cell-class-name="tdCell" height="501" append>
        ...
        <el-table-column label="含税价(元)">
          <template slot-scope="scope">
            <input v-model="scope.row.vatPrice" type="text" class="inputCell tx-r" :disables="userSalePriceType + scope.row.salePriceType <= 1" @change="calculate(scope.$index)">
          </template>
        </el-table-column>
      ...
      <div class="tx-c" style="margin-top:15px">
        ...
        <el-button v-else class="submitBtn" type="primary" @click="$store.dispatch('tagsView/delView', $route);$router.replace('/purchase/data')">返 回</el-button>
      </div>
    </div>
...
export default {
  name: 'PurchaseAdd',
  components: { staffList, warehouseList, supplierList, paymentTypeList, itemList, settleTypeList },
  data() {
    return {
      id: '',
  ...
  created() {
    this.$store.dispatch('basedata/getPurchaseSettleType')
      ...
      getPurchaseById(this.id).then(res => {
        ...
        for (var i = 0; i < res.data.data.purchaseLine.length; i++) {
          for (var j = 0; j < this.keys.length; j++) {
            ...
            const val = res.data.data.purchaseLine[i][key]
  ...
  methods: {
    ...
    save() {
      ...
      this.temp.purchaseLine = deleteEmptyProp(this.tableData)
      this.temp.settleTypeDetail = this.settleData
      savePurchase(this.temp).then(res => {
