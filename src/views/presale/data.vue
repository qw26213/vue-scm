<template>
  <div class="app-container">
    <div class="filterDiv">
      ...
      <custList @selectChange="selectChange" />
...
<script>
import { getPresale, delPresale, auditPresale, buildPresaleVoucher, buildReturnedBill, getAuditInfoByHeaderId, printByHeaderId } from '@/api/store'
...
export default {
  ...
  methods: {
    printBill(row) {
      printByHeaderId('/ps/presale', row.id).then(res => {
    ...
    createVouter() {
      ...
      buildPresaleVoucher(obj).then(res => {
        if (res.data.errorCode == 0) {
    ...
    handleScan(id) {
      this.$router.push('/presale/detail?id=' + id)
    },
    handleDel(id) {
