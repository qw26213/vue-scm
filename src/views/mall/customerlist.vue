  ...
  <div class="app-container">
    ...
    <el-dialog :close-on-click-modal="false" title="客户详情" :visible.sync="dialogFormVisible" width="600px">
        <div ref="dataForm" style="width:100%;">
            <div>客户信息</div>
            <div class="info" style="width:100%"><label style="margin-right:10px">客户名称:</label>{{ customerInfo.nickName }}</div>
            <div class="info" style="width:100%"><label style="margin-right:10px">联系电话:</label>{{ customerInfo.tel }}</div>
            <div style="margin-top:20px">地址信息</div>
            <div v-for="(item,index) in customerInfo.addrLine" :key="index">
              <div class="info"><label>收货人：</label><span>{{ item.contact }}</span><span>({{ item.tel }})</span><span v-if="item.isDefault == 1" style="margin-left:15px">(默认)</span></div>
              <div class="info"><label>地址：</label>{{ item.province }}{{ item.city }}{{item.district}}{{item.addr}}</div>
            </div>
        </div>
    </el-dialog>
  </div>
...
<style scoped>
  ...
  .itemUrl{width:48px; height:48px; border:1px #e5e5e5 solid;display: inline-block;vertical-align: middle;margin-right: 10px;border-radius: 50%}
