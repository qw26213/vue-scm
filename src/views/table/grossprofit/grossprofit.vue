...
<script>
import { getAggregate, getGrossProfitTable, getGrossProfitQueryConfList, getDefaultGrossProfitQueryConf, saveGrossProfitQueryConf, delGrossProfitTemplate, exportGrossProfitBook, printGrossProfitBook } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    delItem(id) {
      delGrossProfitTemplate(id).then(res => {
    ...
    saveTemplate(obj) {
      ...
      saveGrossProfitQueryConf(data).then(res => {
        if (res.data.errorCode === '0') {
          this.getTemplateList()
    ...
    getTemplateList() {
      getGrossProfitQueryConfList().then(res => {
        this.templatelist = res.data.data || []
      })
    },
    exportBook() {
      exportGrossProfitBook(this.listQuery)
    },
    printBook() {
    ...
    getList() {
      ...
      getGrossProfitTable(this.listQuery).then(res => {
        this.listLoading = false
