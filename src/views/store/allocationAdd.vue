    ...
    <div class="contentDiv">
      <el-table :data="tableData" border fit highlight-current-row style="width: 100%;" size="small" cell-class-name="tdCell">
        ...
        <el-table-column label="数量" width="90">
          <template slot-scope="{row}">
            <input v-model="row.qty" type="text" class="inputCell tx-r" @focus="focusThis($event)">
          </template>
        </el-table-column>
        ...
        <el-table-column label="备注">
          <template slot-scope="{row}">
            <input v-model="row.remarks" type="text" class="inputCell tx-l" @focus="focusThis($event)">
          </template>
        </el-table-column>
      </el-table>
