<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%;" size="mini" show-summary :span-method="arraySpanMethod">
      ...
      <el-table-column label="借方(期初余额)">
        <template slot-scope="{row}">
          <span>{{row.beginBalanceDr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="贷方(期初余额)">
        <template slot-scope="{row}">
          <span>{{row.beginBalanceCr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="借方(本期发生额)">
        <template slot-scope="{row}">
          <span>{{row.periodNetDr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="贷方(本期发生额)">
        <template slot-scope="{row}">
          <span>{{row.periodNetCr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="借方(本年累计发生额)">
        <template slot-scope="{row}">
          <span>{{row.yearNetDr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="贷方(本年累计发生额)">
        <template slot-scope="{row}">
          <span>{{row.yearNetCr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="借方(期末余额)">
        <template slot-scope="{row}">
          <span>{{row.balanceDr}}</span>
        </template>
      </el-table-column>
      <el-table-column label="贷方(期末余额)">
        <template slot-scope="{row}">
