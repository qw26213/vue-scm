      ...
      <el-table :key="tableKey" v-loading="listLoading" :data="tableData" size="small" border fit highlight-current-row>
        ...
        <el-table-column label="操作" align="left" width="260">
          <template slot-scope="{row}">
            ...
            <span v-if="row.status==1&&(row.returnedType==0||row.returnedType==1)" class="ctrl" @click="handleCreateBill(row.isOutboundOrderReturned,row.id,row.outboundOrderReturnedHeaderId)">{{ row.isOutboundOrderReturned==0?'生成':'查看' }}退货入库单</span>
            <span v-if="row.status==1&&(row.returnedType==2)" class="ctrl" @click="handleCreateBill1(row.isInventory,row.id,row.inventoryId)">{{ row.isInventory==0?'生成':'查看' }}报损单</span>
            <span v-if="row.status==1" class="ctrl" @click="handleCreateVouter(row.isJeHeader,row.id,row.jeHeaderId)">{{ row.isJeHeader==0?'生成':'查看' }}销售退货凭证</span>
...
<script>
import { getSalesReturned, delSalesReturned, auditSalesReturned, buildSalesReturned, buildSaleReturnedVoucherByHeaderId, buildInventoryByHeaderId, getAuditInfoByHeaderId, printByHeaderId } from '@/api/sale'
