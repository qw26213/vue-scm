<template>
  <div class="app-container">
    ...
    <div class="contentDiv" v-show="activeTab == 1">
      <el-table :key="tableKey" v-loading="listLoading" :data="pageData" cell-class-name="tpCell" border fit highlight-current-row style="width: 100%;" size="small">
        ...
        <el-table-column label="借方">
          <el-table-column label="数量">
            <template slot-scope="{row}">
              <p v-for="(item,index) in row.listNetQtyDr" :key="index" class="pCell" style="text-align:right">{{ item | zeroNull }}</p>
            </template>
          </el-table-column>
        ...
        <el-table-column label="贷方">
          <el-table-column label="数量">
            <template slot-scope="{row}">
              <p v-for="(item,index) in row.listNetQtyCr" :key="index" class="pCell" style="text-align:right">{{ item | zeroNull }}</p>
            </template>
          </el-table-column>
          <el-table-column label="金额">
            <template slot-scope="{row}">
        ...
        <el-table-column label="余额">
          <el-table-column label="数量">
            <template slot-scope="{row}">
              <p v-for="(item,index) in row.listBalanceQty" :key="index" class="pCell" style="text-align:right">{{ item | zeroNull }}</p>
            </template>
          </el-table-column>
          <el-table-column label="单价">
            <template slot-scope="{row}">
              <p v-for="(item,index) in row.listPrice" :key="index" class="pCell" style="text-align:right">{{ item | Fixed }}</p>
            </template>
          </el-table-column>
          <el-table-column label="金额">
            <template slot-scope="{row}">
