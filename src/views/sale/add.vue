    ...
    <div class="contentDiv">
      <el-table :data="tableData" border fit highlight-current-row style="width: 100%;" size="small" cell-class-name="tdCell">
        ...
        <el-table-column label="保质期(天)">
          <template slot-scope="{row}">
            <input v-model="row.qualityDays" type="text" @focus="focusThis($event)" class="inputCell tx-r">
          </template>
        </el-table-column>
      ...
      <div class="tx-c" style="margin-top:15px">
        ...
        <el-button v-else class="submitBtn" type="primary" @click="$store.dispatch('tagsView/delView', $route);$router.replace('/sale/data')">返 回</el-button>
      </div>
    </div>
        ...
        if (res.data.data) {
          ...
          for (var i = 0; i < res.data.data.salesLine.length; i++) {
            for (var j = 0; j < this.keys.length; j++) {
              ...
              const val = res.data.data.salesLine[i][key]
