      ...
      <el-table :key="tableKey" v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%;" size="small">
        ...
        <el-table-column label="操作" align="center" width="280">
          <template slot-scope="{row}">
            ...
            <span v-if="row.status==1" class="ctrl" @click="handleCreateBill(row.isWarehousingEntry,row.id,row.warehousingEntryHeaderId)">{{ row.isWarehousingEntry==0?'生成':'查看' }}入库单</span>
            <span v-if="row.status==1" class="ctrl" @click="handleCreateVouter(row.isJeHeader,row.id,row.jeHeaderId)">{{ row.isJeHeader==0?'生成':'查看' }}采购凭证</span>
...
<script>
import { getPurchase, delPurchase, auditPurchase, buildPurchaseEntry, buildVoucherByHeaderId, getAuditInfoByHeaderId, printByHeaderId } from '@/api/store'
...
export default {
  ...
  methods: {
    ...
    createVouter() {
      var obj = { isBillDate: this.isBillDate, id: this.curBillId }
      buildVoucherByHeaderId(obj).then(res => {
        if (res.data.errorCode == 0) {
