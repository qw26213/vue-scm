...
<script>
import { getAggregate, getPurchaseTable, getPurchaseQueryConfList, getDefaultPurchaseQueryConf, savePurchaseQueryConf, delPurchaseTemplate, exportPurchaseBook, printPurchaseBook } from '@/api/table'
...
export default {
    ...
    created() {
      ...
      getAggregate('purchase').then(res => {
        this.levellist = res.data
        getDefaultPurchaseQueryConf().then(res => {
    ...
    methods: {
        ...
        delItem(id) {
            delPurchaseTemplate(id).then(res => {
        ...
        saveTemplate(obj) {
            ...
            savePurchaseQueryConf(data).then(res => {
                if (res.data.errorCode == 0) {
                    this.getTemplateList()
        ...
        getTemplateList() {
            getPurchaseQueryConfList().then(res => {
                this.templatelist = res.data.data || []
            })
        },
        exportBook() {
            exportPurchaseBook(this.listQuery)
        },
        printBook() {
            printPurchaseBook(this.listQuery).then(res => {
        ...
        getList() {
            ...
            getPurchaseTable(this.listQuery).then(res => {
                this.listLoading = false
