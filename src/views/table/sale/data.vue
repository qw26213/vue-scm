...
export default {
  ...
  methods: {
    ...
    saveTemplate(obj) {
      ...
      saveSalesQueryConf(data).then(res => {
        if (res.data.errorCode === '0') {
          this.getTemplateList()
    ...
    getTemplateList() {
      getSalesQueryConfList().then(res => {
        this.templatelist = res.data.data || []
      })
    },
    exportBook() {
      exportSalesBook(this.listQuery)
    },
    printBook() {
      printSalesBook(this.listQuery).then(res => {
    ...
    getList() {
      ...
      getSalesTable(this.listQuery).then(res => {
        this.listLoading = false
