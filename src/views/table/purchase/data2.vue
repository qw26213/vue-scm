<template>
  <div class="app-container">
    ...
    <div class="contentDiv">
      ...
      <pagination v-show="total>20" :total="total" :page.sync="listQuery.pageIndex" :limit.sync="listQuery.pageNum" @pagination="pageData" />
    </div>
    <saveSelect :dialogvisible.sync="selectModalVisible" @saveTemplate="saveTemplate" />
  </div>
</template>
<script>
import { getAggregate, getPurchaseReturnedTable, getPurchaseReturnedQueryConfList, getDefaultPurchaseReturnedQueryConf, savePurchaseReturnedQueryConf, delPurchaseReturnedTemplate, exportPurchaseReturnedBook, printPurchaseReturnedBook } from '@/api/table'
...
export default {
  ...
  created() {
    ...
    getAggregate('purchaseReturned').then(res => {
      this.levellist = res.data
      getDefaultPurchaseReturnedQueryConf().then(res => {
  ...
  methods: {
    ...
    delItem(id) {
      delPurchaseReturnedTemplate(id).then(res => {
    ...
    saveTemplate(obj) {
      ...
      savePurchaseReturnedQueryConf(data).then(res => {
        if (res.data.errorCode === '0') {
          this.getTemplateList()
    ...
    getTemplateList() {
      getPurchaseReturnedQueryConfList().then(res => {
        this.templatelist = res.data.data || []
      })
    },
    exportBook() {
      exportPurchaseReturnedBook(this.listQuery)
    },
    printBook() {
      printPurchaseReturnedBook(this.listQuery).then(res => {
    ...
    getList() {
      ...
      getPurchaseReturnedTable(this.listQuery).then(res => {
        this.listLoading = false
